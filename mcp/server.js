...
const SITE = "https://dheeraj-work.netlify.app"
...
server.tool("get_access_terms", "How to pay for AI access", {}, async () =>
  ok({
    amount: "0.01 USDC",
    network: "Base",
...
server.tool(
  "verify_payment",
  "Exchange a Base tx hash for an access token",
  { txHash: z.string() },
  async ({ txHash }) => {
...
server.tool("list_posts", "List post titles and links from the public feed", {}, async () => {
...
server.tool(
  ...
  async ({ path, accessToken }) => {
    const res = await fetch(`${SITE}/api/content?path=${encodeURIComponent(path)}`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    })
    const body = await res.text()
    if (res.status === 402) {
      return ok("Payment required. Call get_access_terms, pay, then verify_payment.")
    }
    return ok(body)
  }
)
